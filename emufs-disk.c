...
int alloc_inode(int mount_point){
	...
	*/
	struct superblock_t* sup;
	read_superblock(mount_point,sup);
	char check[MAX_INODES];
	memcpy(check,sup->inode_bitmap, MAX_INODES);
	int i;
	for(i=0;i<MAX_INODES;i++){
	if(check[i]=='0'){
		sup->inode_bitmap[i]='1';
		sup->used_inodes++;
		write_superblock(mount_point,sup);
		return i;
		}}
	return -1;
}
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	int bl;
	if(inodenum<16 && inodenum >=0)bl=1;
	else if(inodenum<32 && inodenum >=16)bl=2;
	char buff[BLOCKSIZE];
	readblock(mounts[mount_point].device_fd,bl,buff);
	if((mounts[mount_point].fs_number)==1) {decrypt(mounts[mount_point].key,buff,sizeof(buff));}
	memcpy(met,buff,BLOCKSIZE);
	in=&met->inodes[inodenum];
	memcpy(inodeptr,in,sizeof(struct inode_t));
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	memcpy(met,buff,BLOCKSIZE);
	in=&met->inodes[inodenum];
	memcpy(in,inodeptr,sizeof(struct inode_t));
	char buff2[BLOCKSIZE];
	memcpy(buff2,met,sizeof(struct metadata_t));
	if((mounts[mount_point].fs_number)==1) {encrypt(mounts[mount_point].key,buff2,sizeof(buff2));}
	writeblock(mounts[mount_point].device_fd,bl,buff2);
...
int alloc_datablock(int mount_point){
	...
	*/
	struct superblock_t* sup;
	read_superblock(mount_point,sup);
	char check[MAX_BLOCKS];
	memcpy(check,sup->block_bitmap, MAX_BLOCKS);
	int i;
	for(i=0;i<MAX_BLOCKS;i++){
	if(check[i]=='0'){
		sup->block_bitmap[i]='1';
		sup->used_blocks++;
		write_superblock(mount_point,sup);
		return i;
		}}
...
void read_datablock(int mount_point, int blocknum, char *buf){
	...
	*/
	char buff[BLOCKSIZE];
	readblock(mounts[mount_point].device_fd,blocknum,buff);
	if((mounts[mount_point].fs_number)==1) {decrypt(mounts[mount_point].key,buff,sizeof(buff));}\
	memcpy(buff,buf,BLOCKSIZE);
...
void write_datablock(int mount_point, int blocknum, char *buf){
	...
	*/
	char buff[BLOCKSIZE];
	memcpy(buff,buf,BLOCKSIZE);
	if((mounts[mount_point].fs_number)==1) {encrypt(mounts[mount_point].key,buff,sizeof(buff));}
	writeblock(mounts[mount_point].device_fd,blocknum,buff);
