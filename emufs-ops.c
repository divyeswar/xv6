...
void emufs_close(int handle, int type){
    /*
        ...
        * 
    */
   if(type==0)files[handle].mount_point=-1;
   else if(type==1)dir[handle].mount_point=-1;
}
...
int emufs_create(int dir_handle, char* name, int type){
   ...
   int i;
   for(i=0;i<4;i++){
    if(par_inode->mappings[i]==-1)break;
    if(i==3) return -1;
   }
